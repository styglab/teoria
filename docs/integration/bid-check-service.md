...
동일한 회사 근거·공고 요건 추출본·기준일·참여방식·Registry·평가 규칙으로 만든 공고별
`assessment_fingerprint` 결과는 프로세스별로 5분간 캐시하며 최대 2,048건을 보관한다. 요건 추출본이나
회사 근거가 바뀌면 fingerprint도 바뀌므로 이전 판정을 재사용하지 않는다. 이는 짧은 시간의 페이지
왕복과 재시도를 줄이는 캐시다. 회사 공통 근거와 기준일별 근거 조회도 같은 TTL로 캐시해 fingerprint
검증을 위한 외부 원천 재호출을 줄인다.
