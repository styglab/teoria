...
from teoria.execution.source.executor import MissingCredentialError, SourceExecutor
...
from teoria.verification.source.state import SourceVerificationState
...
def create_source_verification_graph(services: SourceVerificationServices | None = None):
    ...
    async def execute_request(state: SourceVerificationState) -> dict[str, Any]:
        ...
        try:
            ...
            diagnostic = Diagnostic("source_connection_error", str(exc), Path(state["registry_root"]), location=request.operation_id)
