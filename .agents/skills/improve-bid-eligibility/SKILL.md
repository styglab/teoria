...
14. If an acceptance threshold fails and another safe, reproducible improvement is available, continue with the next iteration instead of stopping after one report. Move failed holdout cases into the next discovery/regression cycle, freeze a newly sampled holdout before inspecting it, and use a new deterministic seed. Stop only when the requested gate passes, the user stops the run, or progress is genuinely blocked by unavailable source data, credentials, capacity, or an ambiguity requiring external adjudication.
