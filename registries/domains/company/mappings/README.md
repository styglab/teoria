...
Binding에는 내부 식별자, 고정값이나 수집시각을 원천 필드처럼 넣지 않는다. 객체·링크 생성 규칙은 별도의 `materializations`에서 선언하고 실제 값 생성은 Runtime이 담당한다.

도메인별 Mapping은 `registries/domains/{domain}/mappings/`에 두며 `ontology`를 기준으로 구성한다. 여러 Source의 binding이 같은 Ontology Property 아래에 함께 존재할 수 있다.
...
Registry에는 `teoria.transforms` 접두사를 쓰지 않는다. 예를 들어 `company.normalize_representative_names`는 런타임에서 `teoria.transforms.company.normalize_representative_names` 함수로 해석된다.
