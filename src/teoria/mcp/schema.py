...
from teoria.models.capability import CapabilityDefinition, CapabilityInput
from teoria.models.ontology import OntologyProperty
...
def capability_input_schema(catalog: RegistryCatalog, capability: CapabilityDefinition) -> dict[str, Any]:
    properties = {
        input_id: _input_schema(catalog, definition)
        for input_id, definition in capability.inputs.items()
    }
    properties["_options"] = {
        "type": "object",
        "description": "MCP 응답 표현 옵션이며 원천 API 요청에는 전달되지 않는다.",
        "properties": {
            "include_property_provenance": {
                "type": "boolean",
                "default": False,
                "description": "각 속성의 상세 출처를 응답에 포함한다.",
            },
            "max_objects": {
                "type": "integer",
                "minimum": 1,
                "maximum": 1000,
                "default": 200,
                "description": "응답에 포함할 최대 객체 수이다.",
            },
        },
        "additionalProperties": False,
    }
    required = [input_id for input_id, definition in capability.inputs.items() if definition.required]
    schema: dict[str, Any] = {
        "type": "object",
...
def coerce_capability_inputs(
    ...
    return {
        input_id: _coerce_input(catalog, definition, arguments[input_id])
        for input_id, definition in capability.inputs.items()
        if input_id in arguments
    }
...
def _type_schema(
    ...
    if property_definition and property_definition.value_set:
        value_set = catalog.value_sets[property_definition.value_set]
        schema: dict[str, Any] = {
            "type": "string",
            "enum": [item.id for item in value_set.values],
        }
...
def _resolve_property(catalog: RegistryCatalog, reference: str | None) -> OntologyProperty:
    ...
    object_type = next(item for item in ontology.object_types if item.id == object_id)
